...
function IsValidUrlAndMonitorProtocol(url) {
  var s_strTraditionSchemeHeaders = "HTTP://HTTPS://FTP://THUNDER://MMS://MMST://RTSP://RTSPU://XLAPP://";
...
function GetConfig() {
  ...
  chrome.extension.sendRequest({
    ...
    function (response) {
      ...
      strMonitorDemain = response.monitorDemains;
      strFilterDemain = response.filterDemains;
      strMonitorFileExt = response.monitorFileExts;
    }
  );
...
function CheckEnabled(url, tabId) {
  chrome.extension.sendRequest({
    name: 'CheckEnabled',
    url: url,
    tabId: tabId
  ...
  chrome.extension.onMessage.addListener(
    function (message, sender, sendResponseCallback) {
      if (message.name == "UpdatePluginEnabled") {
        ...
        // console.log('OnActivated', bLoaded, message, sender);
        if (bLoaded) {
          // 切换回当前页，重新查询enable状态
          CheckEnabled(document.location.href, message.tabId);
...
function Init() {
  ...
  CheckActivated(document.location.href);
